...
export default function DialogComposer({ placeholder = 'Message…', agents = [], users = [], onSend, onSendAudio, disabled, draftKey }: MessageComposerProps) {
  ...
  return (
    <div className="relative"
      ...
      {/* Image / file previews — sit ABOVE the pill so the input stays clean */}
      {(images.length > 0 || attachments.length > 0) && (
        <div className="mb-2 flex flex-wrap gap-2">
          ...
          {attachments.map((file, i) => (
            <div key={`file-${i}`} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 border border-gray-200 group/file">
              ...
              <div className="min-w-0">
                <p className="text-xs font-medium text-gray-700 truncate max-w-[160px]">{file.name}</p>
      ...
      {/* Pill — ChatGPT/Claude-style single rounded container */}
      <div className="flex items-end gap-2 px-2 py-1.5 rounded-3xl border border-gray-200 bg-white shadow-sm hover:shadow-md focus-within:border-gray-300 focus-within:shadow-md transition-shadow">
        {/* Attach button */}
        <button onClick={() => fileInputRef.current?.click()}
          className="p-2 rounded-full text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition-colors shrink-0"
          title="Attach file">
        ...
        {/* Textarea */}
        {recording ? (
          <div className="flex-1 flex items-center gap-3 px-2 py-2.5">
          ...
          <textarea
            ...
            rows={1}
            className="flex-1 px-1 py-2.5 text-[15px] leading-[1.55] outline-none bg-transparent resize-none min-h-[24px] max-h-[200px]"
        ...
        {/* Voice — only when there's no text and onSendAudio is provided */}
        {onSendAudio && !hasContent && !recording && (
        ...
        {recording && (
          <button onClick={toggleRecording}
            className="p-2 rounded-full bg-red-600 text-white hover:bg-red-700 shrink-0"
            title="Stop & send">
            <Square className="h-5 w-5" />
          </button>
        )}

        {/* Send — black circle, ChatGPT/Claude style */}
        <button
          onClick={handleSend}
